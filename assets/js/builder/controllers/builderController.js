angular.module('builder').controller('BuilderController', ['$scope', '$rootScope', '$translate', 'bootstrapper', 'elements', 'settings', 'grid', 'preview', 'themes', 'fonts', 'panels', 'project', '$localStorage', '$http', function($scope, $rootScope, $translate, bootstrapper, elements, settings, grid, preview, themes, fonts, panels, project, $localStorage, $http) {
    $scope.themes = themes;
...
alert('controller');
    ...
    $scope.publish_CCBIZON = function(name) {
      alert('hi exporting');


      console.log("exporting");
      project.getAll().success(function(projects) {
        projects.forEach(function(project) {
         console.log("exportingeee");
          if(project.name === $rootScope.currentProject) {
            console.log("Current project " + angular.toJson(project));
           
           $http.get('export/project/' + project.id).success(function(data) {
            alert('Website published. You can can view it any browser');
           });
        }
      });
      });
      };
      /*$http.post('export/project', payload).success(function() {
			var name = $scope.name;
